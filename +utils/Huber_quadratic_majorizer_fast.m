function [c0_t, c1_t, c2_t, c0_dc, c1_dc, c2_dc, c0_reg, c1_reg, c2_reg,  step_out_for_maj] = Huber_quadratic_majorizer_fast(Xin, Pin, step_basis, niter_maj, N1, N2, Nt, patch_size, patches_operator, A, kdata, beta, f_pot, f_dot_pot, f_weight_pot)

% Compute quadratic majorizer coefficients for Huber-based optimization using fast heuristic approach.
%
% Input:
...
for m = 1:niter_maj
    ...
    patches_X = patches_operator(X_basis); % dims: [patch_side^2, #time frames, # patches in each image]
    patches_P = patches_operator(P);
    ...
    c0_reg = (numel(shifts)^2)*c0_reg; % We scale by (numel(shifts)^2) in order to compensate for not having shifts
    c1_reg = (numel(shifts)^2)*c1_reg;
    c2_reg = (numel(shifts)^2)*c2_reg;
